...
function creatw() {
    ...
    function initWebSocket() {
        ...
        if ("WebSocket" in window){
            ...
            if(wss){
                websocket = new WebSocket('wss://'+websocketIp);

            }else{
                websocket = new WebSocket('ws://'+websocketIp);
            }
            websocket.onopen = function () {
                //连接成功
                win.setTitle(title + '&nbsp;&nbsp;(已连接)   【现在全局对话】');
                websocket.send('connectionsSuccess' + user);
				isConnection = true;
            }
    ...
    win.on("close", function () {
		//alert("close");
        //websocket.send('LeaveConnections');
        //isCreatw = false;
    });
    ...
    function send() {
        ...
        if (toUser != "") {
            content = "Sender" + toUser + "Recipient" + content;
        }
        var message = {};
        if (websocket != null) {
            if (input.getValue()) {
